...
from verb_conjugate import verb_conjugate
...
class expressionTranslate:
    ...
    def __init__(self, vorbose=False, model_path='holo', instance=None):
        ...
        self.ModelsPath = os.path.dirname(__file__) + 'models/' + self.model_path #sample2を起動時は/models/
    ...
    def execSudachiTokenizer(self, text):
        ...
        #for self.tokens in self.instance.tokenize('C', text):
       	for self.tokens in self.instance.tokenize(text):
            __list_info = [
                self.tokens.surface(),
...
if __name__ == '__main__':
    ...
    #追加
    __sudachi_resorce = config.DEFAULT_RESOURCEDIR
    
    print(__sudachi_settings)
    print(__sudachi_resorce)
    #print("\n\n\n\n\n")
    
    # init dictionary
    #__dict = dictionary.Dictionary(__sudachi_settings)
    #__dict = dictionary.Dictionary(__sudachi_settings, __sudachi_resorce)
    __dict = dictionary.Dictionary()
    __instance = __dict.create()

    #文字コードの関係でファイル読み込みに変更
